  ...
  try {
    ...
    for (const step of plan) {
      ...
      if (step.data.kind === "source") {
        if (!step.data.sourcePath || !existsSync(step.data.sourcePath)) {
          throw new Error(`Source SVG does not exist: ${step.data.sourcePath}`);
        }

        artifactByNode.set(step.id, {
          inputPath: step.data.sourcePath,
          outputPath: step.data.sourcePath,
          status: "completed",
...
export async function createApp(): Promise<express.Express> {
  ...
  app.get("/api/fs/list", async (request, response, next) => {
    try {
      const dir = String(request.query.dir ?? "");
      response.json({ dir, entries: await listDirectory(dir) });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/fs/read", async (request, response, next) => {
    try {
      ...
      response.json({ path: filePath, content: await fs.readFile(filePath, "utf8") });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/fs/write", async (request, response, next) => {
    try {
      const filePath = String(request.body.path ?? "");
      const content = String(request.body.content ?? "");
      await fs.mkdir(path.dirname(filePath), { recursive: true });
      await fs.writeFile(filePath, content, "utf8");
      response.json({ path: filePath });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/fs/svg", async (request, response, next) => {
    try {
      const filePath = String(request.query.path ?? "");
      response.type("image/svg+xml").send(await fs.readFile(filePath, "utf8"));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/fs/import", async (request, response, next) => {
    try {
      ...
      const projectDir = String(request.body.projectDir ?? "");
      response.json({ importedPath: await copySvgIntoProject(sourcePath, projectDir) });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/graph/load", async (request, response, next) => {
    try {
      const filePath = String(request.query.path ?? "");
      const graph = graphDocumentSchema.parse(JSON.parse(await fs.readFile(filePath, "utf8")));
      response.json(graph);
    } catch (error) {
      next(error);
    }
  ...
  app.post("/api/graph/save", async (request, response, next) => {
    try {
      const filePath = String(request.body.path ?? "");
      const graph = graphDocumentSchema.parse(request.body.graph);
      await fs.mkdir(path.dirname(filePath), { recursive: true });
  ...
  app.post("/api/jobs/run", async (request, response, next) => {
    try {
      ...
      jobs.set(jobId, job);
      void executeGraphJob(jobs, jobId, payload.graph, payload.projectDir, payload.targetNodeId);
