...
void NTHUR::RangeRouter::range_router(Two_pin_element_2d& two_pin, int version) {
    if (!congestion.check_path_no_overflow(two_pin.path, two_pin.net_id, false)) {
        ...
        SPDLOG_TRACE(log_sp, " {} ", two_pin.toString());
        ...
        if ((find_path_flag == false) || !congestion.check_path_no_overflow(bound_path, two_pin.net_id, true)) {
            ...
            start.x = std::max(0, start.x - size);
            start.y = std::max(0, start.y - size);
            end.x = std::min(construct_2d_tree.rr_map.get_gridx() - 1, end.x + size);
            end.y = std::min(construct_2d_tree.rr_map.get_gridy() - 1, end.y + size);
        ...
        SPDLOG_TRACE(log_sp, "congestion {} ", congestion.congestionMap2d.toString());
    }
}
