...
void test("runPipeline can invoke the real labypath binary when available", { timeout: 120000 }, async (t) => {
  const context = resolveContext(workspaceRoot);
  const sourcePath = path.join(workspaceRoot, "LabyPath", "input", "square_circleorig.svg");
