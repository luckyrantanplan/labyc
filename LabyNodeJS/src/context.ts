...
export function resolveContext(startPath = process.cwd()): RuntimeContext {
  ...
  const fallbackBinaryPath = binaryCandidates[0] ?? path.join(workspaceRoot, "labypath");
...
export function requireWorkspacePath(workspaceRoot: string, candidatePath: string, label: string): string {
...
export function requireSvgPath(workspaceRoot: string, candidatePath: string, label: string): string {
  const resolvedPath = requireWorkspacePath(workspaceRoot, candidatePath, label);
