...
std::vector<Node> colorTwoOpposites() {
    ...
    nodes[1].opposite().push_back(nodes.data());
    ...
    StateSelect ss(nodes[0].opposite());
    nodes[0].setState(ss.getNext());

    StateSelect ss2(nodes[1].opposite());
...
TEST(GraphColoringTest, TwoOppositesGetDifferentStates) {
    ...
    EXPECT_NE(nodes[0].state(), -1) << "Node 0 should be colored";
    EXPECT_NE(nodes[1].state(), -1) << "Node 1 should be colored";
    EXPECT_NE(nodes[0].state(), nodes[1].state()) << "Opposite nodes must have different states";
}

TEST(GraphColoringTest, TwoOppositesUseStates0And1) {
    auto nodes = colorTwoOpposites();
    // With 2 opposites and greedy allocation, states should be 0 and 1
    EXPECT_TRUE((nodes[0].state() == 0 && nodes[1].state() == 1) ||
                (nodes[0].state() == 1 && nodes[1].state() == 0));
}
...
TEST(GraphColoringTest, ThreeOpposites) {
    ...
    nodes[0].opposite().push_back(&nodes[1]);
    nodes[0].opposite().push_back(&nodes[2]);
    nodes[1].opposite().push_back(nodes.data());
    nodes[1].opposite().push_back(&nodes[2]);
    nodes[2].opposite().push_back(nodes.data());
    nodes[2].opposite().push_back(&nodes[1]);

    // Color greedily
    StateSelect ss0(nodes[0].opposite());
    nodes[0].setState(ss0.getNext());

    StateSelect ss1(nodes[1].opposite());
    nodes[1].setState(ss1.getNext());

    StateSelect ss2(nodes[2].opposite());
    nodes[2].setState(ss2.getNext());

    // All three states must be different
    EXPECT_NE(nodes[0].state(), nodes[1].state());
    EXPECT_NE(nodes[0].state(), nodes[2].state());
    EXPECT_NE(nodes[1].state(), nodes[2].state());
}
...
TEST(GraphColoringTest, AdjacentNodePreference) {
    ...
    Node n1(0);
    Node n2(1);
    n1.adjacents().insert(&n2);
    n2.adjacents().insert(&n1);

    // Assign state 0 to n1
    n1.setState(0);

    // Per the algorithm, adjacent of state-0 gets state 1
    int32_t adjstate = (n1.state() == 0) ? 1 : 0;
    n2.setState(adjstate);

    EXPECT_EQ(n1.state(), 0);
    EXPECT_EQ(n2.state(), 1);
}
...
TEST(GraphColoringTest, NodeVisitedFlagLifecycle) {
    Node n(0);
    EXPECT_EQ(n.visited(), 0); // unvisited

    n.setVisited(-1); // BFS-marked
    EXPECT_EQ(n.visited(), -1);

    n.setVisited(1); // visited
    EXPECT_EQ(n.visited(), 1);
}
...
TEST(GraphColoringTest, NodeOverlapSortByState) {
    ...
    EXPECT_EQ(nodes[0]->state(), 0);
    EXPECT_EQ(nodes[1]->state(), 1);
    EXPECT_EQ(nodes[2]->state(), 2);
}
