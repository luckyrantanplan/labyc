...
public:
    ...
    static void Shift(Complex *data, unsigned int nx, unsigned int ny, unsigned int threads) {
...
#pragma omp parallel for num_threads(threads)
    ...
    static void Shift(double *data, unsigned int nx, unsigned int ny, unsigned int threads) {
        if (nx % 2 == 0) {
...
#pragma omp parallel for num_threads(threads)
    ...
    static void Shift(Complex *data, unsigned int nx, unsigned int ny, unsigned int nz, unsigned int threads) {
...
#pragma omp parallel for num_threads(threads)
    ...
    static void Shift(double *data, unsigned int nx, unsigned int ny, unsigned int nz, unsigned int threads) {
...
#pragma omp parallel for num_threads(threads)
    ...
    fftw() :
            doubles { }, sign { }, threads { }, norm { }, plan(NULL), inplace { } {
    }
    fftw(unsigned int doubles, int sign, unsigned int threads, unsigned int n = 0) :
            doubles(doubles), sign(sign), threads(threads), norm(1.0 / (n ? n : doubles / 2)), plan(NULL), inplace { } {
#ifndef FFTWPP_SINGLE_THREAD
        fftw_init_threads();
#endif      
    }
    ...
    inline void CheckAlign(Complex *p, const char *s) {
        if ((size_t) p % sizeof(Complex) == 0)
            return;
        std::cerr << "WARNING: " << s << " array is not " << sizeof(Complex) << "-byte aligned: address " << p << std::endl;
    }
...
public:
    ...
    Transpose(unsigned int rows, unsigned int cols, unsigned int length, T *in, T *out = NULL, unsigned int threads = fftw::maxthreads) :
        ...
        if (size % sizeof(double) != 0) {
            std::cerr << "ERROR: Transpose is not implemented for type of size " << size;
            exit(1);
        }
        plan = plan2 = NULL;
        if (rows == 0 || cols == 0)
...
struct keyless1 {
    bool operator()(const keytype1& a, const keytype1& b) const {
        return a.nx < b.nx || (a.nx == b.nx && (a.threads < b.threads || (a.threads == b.threads && a.inplace < b.inplace)));
    }
};
...
struct keyless2 {
    bool operator()(const keytype2& a, const keytype2& b) const {
        return a.nx < b.nx || (a.nx == b.nx && (a.ny < b.ny || (a.ny == b.ny && (a.threads < b.threads || (a.threads == b.threads && a.inplace < b.inplace)))));
    }
};
...
struct keyless3 {
    bool operator()(const keytype3& a, const keytype3& b) const {
        return a.nx < b.nx || (a.nx == b.nx && (a.ny < b.ny || (a.ny == b.ny && (a.nz < b.nz || (a.nz == b.nz && (a.threads < b.threads || (a.threads == b.threads && a.inplace < b.inplace)))))));
    }
};
...
public:
    ...
    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_1d(nx, (fftw_complex *) in, (fftw_complex *) out, sign, effort);
    }
};
...
public:
    ...
    fftwblock(unsigned int nx, unsigned int M, size_t istride, size_t ostride, size_t idist, size_t odist, Complex *in, Complex *out, unsigned int Threads) :
            fftw(), nx(nx), M(M), istride(istride), ostride(ostride), idist(Dist(nx, istride, idist)), odist(Dist(nx, ostride, odist)), plan1(NULL), plan2(NULL) {
        T = 1;
        Q = M;
        R = 0;
    ...
    fftw_plan Plan(int Q, fftw_complex *in, fftw_complex *out) {
        return fftw_plan_many_dft(1, &nx, Q, in, NULL, istride, idist, out, NULL, ostride, odist, sign, effort);
    }

    fftw_plan Plan(int Q, double *in, fftw_complex *out) {
        return fftw_plan_many_dft_r2c(1, &nx, Q, in, NULL, istride, idist, out, NULL, ostride, odist, effort);
    }

    fftw_plan Plan(int Q, fftw_complex *in, double *out) {
        return fftw_plan_many_dft_c2r(1, &nx, Q, in, NULL, istride, idist, out, NULL, ostride, odist, effort);
    }
...
//
//
class mfft1d: public fftwblock<fftw_complex, fftw_complex>, public Threadtable<keytype3, keyless3> {
    static Table threadtable;
public:
    mfft1d(unsigned int nx, int sign, unsigned int M = 1, size_t stride = 1, size_t dist = 0, Complex *in = NULL, Complex *out = NULL, unsigned int threads = maxthreads) :
            fftw(2 * ((nx - 1) * stride + (M - 1) * Dist(nx, stride, dist) + 1), sign, threads, nx), fftwblock<fftw_complex, fftw_complex>(nx, M, stride, stride, dist, dist, in, out, threads) {
    }
    ...
    mfft1d(unsigned int nx, int sign, unsigned int M, size_t istride, size_t ostride, size_t idist, size_t odist, Complex *in = NULL, Complex *out = NULL, unsigned int threads = maxthreads) :
            fftw(std::max(2 * ((nx - 1) * istride + (M - 1) * Dist(nx, istride, idist) + 1), 2 * ((nx - 1) * ostride + (M - 1) * Dist(nx, ostride, odist) + 1)), sign, threads, nx), fftwblock<
                    fftw_complex, fftw_complex>(nx, M, istride, ostride, idist, odist, in, out, threads) {
    }
...
//   out contains the first n/2+1 Complex Fourier values.
//
class mrcfft1d: public fftwblock<double, fftw_complex>, public Threadtable<keytype3, keyless3> {
    static Table threadtable;
public:
    mrcfft1d(unsigned int nx, unsigned int M, size_t istride, size_t ostride, size_t idist, size_t odist, double *in = NULL, Complex *out = NULL, unsigned int threads = maxthreads) :
            fftw(std::max((realsize(nx, in, out) - 2) * istride + (M - 1) * idist + 2, 2 * (nx / 2 * ostride + (M - 1) * odist + 1)), -1, threads, nx), fftwblock<double, fftw_complex>(nx, M, istride,
                    ostride, idist, odist, (Complex *) in, out, threads) {
    }
...
//   out contains the n real values stored as a Complex array.
//
class mcrfft1d: public fftwblock<fftw_complex, double>, public Threadtable<keytype3, keyless3> {
    static Table threadtable;
public:
    mcrfft1d(unsigned int nx, unsigned int M, size_t istride, size_t ostride, size_t idist, size_t odist, Complex *in = NULL, double *out = NULL, unsigned int threads = maxthreads) :
            fftw(std::max(2 * (nx / 2 * istride + (M - 1) * idist + 1), (realsize(nx, in, out) - 2) * ostride + (M - 1) * odist + 2), 1, threads, nx), fftwblock<fftw_complex, double>(nx, M, istride,
                    ostride, idist, odist, in, (Complex *) out, threads) {
    }
...
public:
    ...
    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_r2c_2d(nx, ny, (double *) in, (fftw_complex *) out, effort);
    }
...
public:
    ...
    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_c2r_2d(nx, ny, (fftw_complex *) in, (double *) out, effort);
    }
...
public:
    ...
    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_3d(nx, ny, nz, (fftw_complex *) in, (fftw_complex *) out, sign, effort);
    }
};
...
public:
    ...
    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_r2c_3d(nx, ny, nz, (double *) in, (fftw_complex *) out, effort);
    }
...
public:
    ...
    crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, Complex *in, double *out = NULL, unsigned int threads = maxthreads) :
            fftw(nx * ny * (realsize(nz, in, out)), 1, threads, nx * ny * nz), nx(nx), ny(ny), nz(nz) {
        Setup(in, out);
    }

    fftw_plan Plan(Complex *in, Complex *out) {
        return fftw_plan_dft_c2r_3d(nx, ny, nz, (fftw_complex *) in, (double *) out, effort);
    }
