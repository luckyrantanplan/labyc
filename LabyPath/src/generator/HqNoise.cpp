...
auto HqNoise2D::get(const double xCoordinate, const double yCoordinate) const -> double {
    ...
    double const xInterpolatedHigh =
        HqNoiseUtils::lerp(_array(xBase, yNext).real(), _array(xNext, yNext).real(), xFraction);

    return HqNoiseUtils::lerp(xInterpolatedLow, xInterpolatedHigh, yFraction);
}

auto HqNoise2D::getComplex(const std::complex<double>& complexPoint) const -> std::complex<double> {
    return getComplex(complexPoint.real(), complexPoint.imag());
}

auto HqNoise2D::getComplex(const double xCoordinate,
                           const double yCoordinate) const -> std::complex<double> {
    double const xScaled = xCoordinate * _config.accuracy;
    double const yScaled = yCoordinate * _config.accuracy;
...
void HqNoise2D::normalize(uint32_t xSampleCount, uint32_t ySampleCount) {
    ...
    double mid = (maxVal + minVal) / kTwo;
    double maxAmpl = kTwo * _config.amplitude / (maxVal - minVal);
    ...
    if (_config.complex) {
        ...
        mid = (maxVal + minVal) / kTwo;
        maxAmpl = kTwo * _config.amplitude / (maxVal - minVal);
        for (uint32_t yIndex = 0; yIndex < ySampleCount; ++yIndex) {
            for (uint32_t xIndex = 0; xIndex < xSampleCount; ++xIndex) {
                double const normalizedValue =
...
HqNoise2D::HqNoise2D(const HqNoiseConfig& noiseConfig)
    : _config{noiseConfig}, _array{noiseConfig.maxN * noiseConfig.accuracy + 2,
    ...
    fft::Plan forward{fftw_plan_dft_2d(static_cast<int>(xSampleCount),
                                       static_cast<int>(ySampleCount), _array.raw(), _array.raw(),
                                       FFTW_FORWARD, FFTW_ESTIMATE)};
    fft::Plan backward{fftw_plan_dft_2d(static_cast<int>(xSampleCount),
                                        static_cast<int>(ySampleCount), _array.raw(), _array.raw(),
    ...
    for (uint32_t xIndex = 0; xIndex < xSampleCount; ++xIndex) {
        for (uint32_t yIndex = 0; yIndex < ySampleCount; ++yIndex) {
            ...
            if (_config.complex) {
                _array(xIndex, yIndex) = std::complex<double>(
                    realSample * sgn(xIndex, yIndex), distribution.get() * sgn(xIndex, yIndex));
            } else {
    ...
    double const power = _config.powerlaw.power * 0.5;
    double const pfreq = _config.powerlaw.frequency;
    double const quot = pfreq * pfreq;

    double const gfreq = _config.gaussian.frequency;
    double const gaussVariance = kTwo * gfreq * gfreq;

    // cut freq 0
    _array(xSampleCount / 2, ySampleCount / 2) = 0.0;

    for (uint32_t xIndex = 0; xIndex < xSampleCount; ++xIndex) {
        for (uint32_t yIndex = 0; yIndex < ySampleCount; ++yIndex) {
            double const xDistance = static_cast<double>(xIndex) - xSampleCount / kTwo;
            double const yDistance = static_cast<double>(yIndex) - ySampleCount / kTwo;
...
HqNoise1D::HqNoise1D(const HqNoiseConfig& noiseConfig)
    ...
    // Create FFTW plans before filling data (FFTW_ESTIMATE does not touch the array)
    fft::Plan forward{fftw_plan_dft_1d(static_cast<int>(sampleCount), _array.raw(), _array.raw(),
                                       FFTW_FORWARD, FFTW_ESTIMATE)};
    fft::Plan backward{fftw_plan_dft_1d(static_cast<int>(sampleCount), _array.raw(), _array.raw(),
                                        FFTW_BACKWARD, FFTW_ESTIMATE)};

    for (uint32_t sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex) {
        double const realSample = distribution(generator);
        if (_config.complex) {
            _array(sampleIndex) = std::complex<double>(realSample * sgn(sampleIndex),
                                                       distribution(generator) * sgn(sampleIndex));
        } else {
    ...
    for (uint32_t sampleIndex = 0; sampleIndex < sampleCount; ++sampleIndex) {
        double const distanceFromCenter = static_cast<double>(sampleIndex) - sampleCount / kTwo;
        ...
        if (gaussVariance > 0) {
            _array(sampleIndex) *=
                std::exp(-(distanceFromCenter * distanceFromCenter) / gaussVariance);
        }
    }
