...
auto validateValueCount(std::size_t actualCount, const ComplexField2DMeta& meta) -> void {
    const auto expectedCount = static_cast<std::size_t>(meta.width) *
...
auto writeComplexField(const std::string& path, const ComplexField2D& field) -> void {
    ...
    validateValueCount(
        field.values.num_elements() * static_cast<std::size_t>(kComplexComponentCount), field.meta);
    ...
    message.mutable_values()->Reserve(static_cast<int>(
        field.values.num_elements() * static_cast<std::size_t>(kComplexComponentCount)));
