...
struct QueueNodePolyConvex {
    ...
    QueueNodePolyConvex(Node& n, const PolyConvex& pc) :
            _n { n }, _pc { pc } {
    }
...
private:
    ...
    void reCutAllGeometry(const std::vector<const Family*>& families, const std::vector<PolyConvex>& polyConvexList, const std::unordered_map<std::size_t, Node*>& map);
    ...
    void unify(std::size_t second, const std::vector<std::size_t>& aAdjs, std::unordered_set<Intersection>& intersections, CGAL::Union_find<size_t>& uf, const Intersection& i);
    std::vector<Node> processFamilies(std::unordered_map<size_t, std::size_t>& families, const std::vector<PolyConvex>& polyConvexList, std::vector<Family>& familyVector,
            std::vector<Intersection>& intersections, std::vector<BoxIntersection>& box_intersection, CGAL::Union_find<std::size_t>& uf);
    void mergeFamilies(const std::vector<const Family*>& families, std::unordered_set<Intersection>& intersectOnSinglePiece, const std::vector<PolyConvex>& polyConvexList, std::vector<Node>& nodes);
    ...
    std::unordered_map<std::size_t, std::vector<const Family*> > locateFamilies(const std::unordered_map<std::size_t, std::size_t>& families, std::vector<Family>& familyVector,
    ...
    std::vector<Node> createNode(const std::unordered_map<std::size_t, std::vector<const Family*> >& map, std::unordered_set<Intersection> intersectOnSinglePiece,
