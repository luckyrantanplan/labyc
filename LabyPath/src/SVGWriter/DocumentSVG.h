...
struct Layout {
    ...
    explicit Layout(Dimensions const & dims =
                        Dimensions(Dimensions::Size{kDefaultDocumentWidth,
...
public:
    explicit Fill(Color::Defaults colorValue) :
        _color(colorValue) {
    }
    explicit Fill(const Color& color = Color(Color::Defaults::Transparent)) :
        _color(color) {
    }
    [[nodiscard]] auto toString(Layout const & layout) const -> std::string override {
        std::stringstream ss;
...
public:
    explicit Font(double fontSize = kDefaultFontSize, std::string  family = "Verdana") :
...
protected:
    [[nodiscard]] auto fillStyle() const -> const Fill& { return _fill; }
    [[nodiscard]] auto strokeStyle() const -> const Stroke& { return _stroke; }
...
public:
    Elipse(Point  centerPoint, double widthValue, double heightValue,
           Fill const & fillValue = Fill(), Stroke const & strokeValue = Stroke()) :
            Shape(fillValue, strokeValue), _center(std::move(centerPoint)), _radius_width(widthValue / 2), _radius_height(heightValue / 2) {
    }
    [[nodiscard]] auto toString(Layout const & layout) const -> std::string override {
        std::stringstream ss;
        ss << elemStart("ellipse") << attribute("cx", translateX(_center, layout)) << attribute("cy", translateY(_center, layout)) << attribute("rx", translateScale(_radius_width, layout))
                << attribute("ry", translateScale(_radius_height, layout)) << fillStyle().toString(layout) << strokeStyle().toString(layout) << emptyElemEnd();
        return ss.str();
    }
    void offset(const laby::Kernel::Vector_2 & off) override {
...
public:
    Rectangle(Point  edgePoint, double widthValue, double heightValue,
              Fill const & fillValue = Fill(), Stroke const & strokeValue = Stroke()) :
            Shape(fillValue, strokeValue), _edge(std::move(edgePoint)), _width(widthValue), _height(heightValue) {
    }
    [[nodiscard]] auto toString(Layout const & layout) const -> std::string override {
        std::stringstream ss;
        ss << elemStart("rect") << attribute("x", translateX(_edge, layout)) << attribute("y", translateY(_edge, layout)) << attribute("width", translateScale(_width, layout))
                << attribute("height", translateScale(_height, layout)) << fillStyle().toString(layout) << strokeStyle().toString(layout) << emptyElemEnd();
        return ss.str();
    }
    void offset(const laby::Kernel::Vector_2 & off) override {
...
public:
    Line(Point  startPoint, Point  endPoint, Stroke const & strokeValue = Stroke()) :
            Shape(Fill(), strokeValue), _start_point(std::move(startPoint)), _end_point(std::move(endPoint)) {
    }
    [[nodiscard]] auto toString(Layout const & layout) const -> std::string override {
        std::stringstream ss;
        ss << elemStart("line") << attribute("x1", translateX(_start_point, layout)) << attribute("y1", translateY(_start_point, layout)) << attribute("x2", translateX(_end_point, layout))
                << attribute("y2", translateY(_end_point, layout)) << strokeStyle().toString(layout) << emptyElemEnd();
        return ss.str();
    }
    void offset(const laby::Kernel::Vector_2 & off) override {
        _start_point += off;
...
public:
    ...
    explicit Polygon(Stroke const & strokeValue = Stroke()) :
        Shape(Fill(), strokeValue) {
    }
...
public:
    ...
    explicit Path(Stroke const & strokeValue = Stroke()) :
            Shape(Fill(), strokeValue) {
        startNewSubPath();
    }
...
public:
    ...
    explicit Polyline(Stroke const & strokeValue = Stroke()) :
        Shape(Fill(), strokeValue) {
    }
    explicit Polyline(std::vector<Point> const & pointsValue, Fill const & fillValue = Fill(), Stroke const & strokeValue = Stroke()) :
...
public:
    Text(Point  originPoint, std::string  textContent, Fill const & fillValue = Fill(), Font  fontValue = Font(), Stroke const & strokeValue = Stroke()) :
        Shape(fillValue, strokeValue), _origin(std::move(originPoint)), _content(std::move(textContent)), _font(std::move(fontValue)) {
    }
    [[nodiscard]] auto toString(Layout const & layout) const -> std::string override {
        std::stringstream ss;
    ss << elemStart("text") << attribute("x", translateX(_origin, layout)) << attribute("y", translateY(_origin, layout)) << fillStyle().toString(layout) << strokeStyle().toString(layout) << _font.toString(layout)
